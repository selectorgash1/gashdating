...
const DashboardScreen: React.FC<DashboardScreenProps> = ({ onSelectUser, siteConfig }) => {
  ...
  return (
    <div className="flex-grow flex flex-col bg-white overflow-x-hidden relative">
        ...
        <div className="fixed inset-0 z-[200] bg-slate-950 flex flex-col items-center justify-center p-8 animate-in fade-in duration-500">
           <div className="relative z-10 text-center space-y-8">
              <div className="flex justify-center -space-x-12">
                 <div className="w-40 h-56 rounded-3xl border-4 border-white/10 overflow-hidden -rotate-12 shadow-2xl"><img src="https://picsum.photos/seed/me/400/600" className="w-full h-full object-cover" /></div>
                 <div className="w-40 h-56 rounded-3xl border-4 border-white/10 overflow-hidden rotate-12 shadow-2xl relative"><img src={user.photos[0]} className="w-full h-full object-cover" /></div>
              </div>
              <div>
              ...
              <button onClick={() => setIsMatching(false)} className="w-full max-w-xs bg-rose-500 text-white font-black py-5 rounded-full shadow-2xl">Initiate Connection</button>
           </div>
        </div>
      )}
      ...
      <div className="flex-grow overflow-y-auto no-scrollbar">
        <div className="px-8 pt-6 pb-2 flex justify-between items-end">
          <div className="space-y-1">
            ...
            <span className="text-3xl font-black text-slate-900 tracking-tighter italic">Global Curations</span>
          </div>
        </div>
      ...
      <div className="absolute bottom-10 left-0 right-0 flex items-center justify-center space-x-6 z-10">
         <button onClick={() => handleAction('pass')} className="w-16 h-16 bg-white border border-slate-100 rounded-full shadow-2xl flex items-center justify-center text-slate-400 active:bg-slate-50 transition-all"><svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M6 18L18 6M6 6l12 12" /></svg></button>
         <button onClick={() => handleAction('like')} className="w-24 h-24 bg-rose-500 rounded-full shadow-2xl flex items-center justify-center text-white scale-110 active:scale-90 transition-all"><svg className="w-10 h-10" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M3.172 5.172a4 4 0 015.656 0L10 6.343l1.172-1.171a4 4 0 115.656 5.656L10 17.657l-6.828-6.829a4 4 0 010-5.656z" clipRule="evenodd" /></svg></button>
         <button onClick={() => handleAction('super_like')} className="w-16 h-16 bg-slate-900 rounded-full shadow-2xl flex items-center justify-center text-rose-500 active:bg-black transition-all"><svg className="w-7 h-7" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M11.3 1.046A1 1 0 0112 2v5h4a1 1 0 01.82 1.573l-7 10A1 1 0 018 18v-5H4a1 1 0 01-.82-1.573l7-10a1 1 0 011.12-.38z" clipRule="evenodd" /></svg></button>
      </div>
    </div>
  );
};
